...
QuakeUiNode::describe_outputs(const merian::NodeIOLayout &io_layout) {
  ...
  con_out = merian::ManagedVkImageOut::color_attachment(
...
void QuakeUiNode::ensure_pipeline(const vk::Format color_format) {
  ...
  pipeline_layout = std::make_shared<merian::PipelineLayout>(
      context,
      std::vector<merian::DescriptorSetLayoutHandle>{descriptor_layout},
      std::vector<vk::PushConstantRange>{pc_range});

  const auto vert = merian::EntryPoint::create(
      context, merian_quake_draw_vert_spv(), merian_quake_draw_vert_spv_size(),
      "main", vk::ShaderStageFlagBits::eVertex);
  const auto frag = merian::EntryPoint::create(
      context, merian_quake_draw_frag_spv(), merian_quake_draw_frag_spv_size(),
      "main", vk::ShaderStageFlagBits::eFragment);

  pipeline = merian::GraphicsPipelineBuilder()
                 .set_vertex_shader(vert)
                 .set_fragment_shader(frag)
                 .vertex_input_add_binding(
...
void QuakeUiNode::ensure_white_texture(const merian::CommandBufferHandle &cmd) {
  ...
  white_texture = allocator->create_texture_from_rgba8(
      cmd, &white_pixel, 1, 1, vk::SamplerAddressMode::eClampToEdge,
      vk::Filter::eNearest, vk::Filter::eNearest, false, "quake_ui_white");
...
void QuakeUiNode::process(
  ...
  const std::shared_ptr<UIDrawCommands> &draw_commands =
      io[con_ui_draw_commands];
  ...
  if (draw_commands->cmds.empty()) {
    cmd->begin_rendering(vk::RenderingInfo{
        {}, vk::Rect2D{{0, 0}, fb_extent}, 1, 0, color_attachment});
    cmd->end_rendering();
    return;
  }
  ...
  for (std::size_t i = 0; i < draw_commands->cmds.size(); ++i) {
    ...
    vertices.push_back(
        {.pos = {x0, y0}, .uv = {q.uv[0], q.uv[1]}, .rgba = cur_color});
    vertices.push_back(
        {.pos = {x1, y0}, .uv = {q.uv[2], q.uv[1]}, .rgba = cur_color});
    vertices.push_back(
        {.pos = {x1, y1}, .uv = {q.uv[2], q.uv[3]}, .rgba = cur_color});
    vertices.push_back(
        {.pos = {x0, y1}, .uv = {q.uv[0], q.uv[3]}, .rgba = cur_color});
  ...
  allocator->get_staging()->cmd_to_device(cmd, vertex_buffer, vertices.data(),
                                          0, vtx_bytes);
  allocator->get_staging()->cmd_to_device(cmd, index_buffer, indices.data(), 0,
                                          idx_bytes);
  ...
  for (const Batch &b : batches) {
    ...
    if (texture->get_image() != last_bound_image) {
      cmd->push_descriptor_set(pipeline,
  ...
  last_stats = Stats{
      ...
      .canvas_events =
          static_cast<uint32_t>(draw_commands->canvas_events.size()),
      .scissor_events =
          static_cast<uint32_t>(draw_commands->scissor_events.size()),
      .color_events = static_cast<uint32_t>(draw_commands->color_events.size()),
      .unique_textures = unique_textures,
  };
}

QuakeUiNode::NodeStatusFlags
QuakeUiNode::properties(merian::Properties &config) {
